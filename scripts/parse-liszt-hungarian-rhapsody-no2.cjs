...
const MIDI_PATH = path.resolve(__dirname, "../public/midi/liz_rhap02.mid");
const OUTPUT_PATH = path.resolve(
  __dirname,
  "../src/content/pieces/lisztHungarianRhapsodyNo2.ts",
);
const EXPECTED_SHA256 =
  "19f62d62836e8df9d760de8cb645e6020a05c444651ad974be62ddef9ef1d654";
...
const division = u16();
if (headerLength !== 6 || format !== 1 || trackCount !== 17 || division !== 480)
  throw new Error(
  ...
  while (pos < end) {
    ...
    if (status === 0xff) {
      ...
      if (kind === 0x90 || kind === 0x80) {
        ...
        throw new Error(
          `unknown MIDI status 0x${status.toString(16)} in track ${trackIndex}`,
        );
    }
  }
...
const trackSummaries = tracks.map((events, trackIndex) => {
  ...
  return {
    ...
    channels: [...new Set(noteOns.map((e) => e.channel))].sort((a, b) => a - b),
...
for (const summary of trackSummaries) {
  const expectedSummary = expected[summary.trackIndex] ?? [0, [], null, null];
  if (
    JSON.stringify([
      summary.noteOns,
      summary.channels,
...
console.log(
  `Selected: tracks=${SELECTED_TRACKS.join(",")} channels=${SELECTED_CHANNELS.join(",")} noteOns=${selected.length} chords=${rawEvents.length} collisions=${collisions} velocities=${velocities.size} maxSimultaneity=${maxSimultaneity}`,
);
